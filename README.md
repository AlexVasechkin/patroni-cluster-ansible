...
```
                  ...
                  └─────────────────────┬──────────────────────┘
                                        │ mutual TLS (https): лидер, конфиг кластера
         ┌──────────────────────────────┼──────────────────────────────┐
         ▼                              ▼                              ▼
┌───────────────────┐        ┌───────────────────┐        ┌───────────────────┐
...
└───────────────────┘        └────────────────────────┘
  ...
  Потоки: etcd ↔ все Patroni-ноды — выбор лидера, mutual TLS (https + client-cert;
              peer-канал между etcd-нодами тоже TLS);
          primary → pg2, pg3 (streaming replication);
          клиенты → pgbouncer1 → текущий primary (auth_query для паролей);
